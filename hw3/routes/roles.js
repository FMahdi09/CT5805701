...
app.get('/', function(req, res) {
	let SQL = "SELECT id as ID, name FROM roles ORDER BY ID ASC";
...
app.get("/:id", function(req, res) {
	let SQL = "SELECT name FROM roles WHERE ID = ?";
	app.connection.execute(SQL, [req.params.id], function(err, data) {
		if (err) {
			console.log("Error fetching data: ", err);
			res.status(404).send(err.sqlMessage);
...
app.put("/:id", function(req, res) {
	let showRow = function() {
		let SQL = "SELECT id, name FROM roles WHERE ID = ?";
		app.connection.execute(SQL, [req.params.id], function(err, data) {
			if (err) {
				console.log("Error fetching data: ", err);
				res.status(404).send(err.sqlMessage);
